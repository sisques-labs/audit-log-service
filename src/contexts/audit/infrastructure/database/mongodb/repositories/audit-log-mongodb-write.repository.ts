...
import { IAuditLogWriteRepository } from '@/contexts/audit/domain/repositories/audit-log-write.repository';
import { AuditLogMongoDbDto } from '@/contexts/audit/infrastructure/database/mongodb/dtos/audit-log-mongodb.dto';
  ...
  implements IAuditLogWriteRepository
{
  ...
  async findById(id: string): Promise<AuditLogAggregate | null> {
    ...
    const doc = (await collection.findOne({
      id,
    })) as unknown as AuditLogMongoDbDto | null;
  ...
  async findByCriteria(
    ...
    this.logger.log(
      `Finding audit logs by criteria: ${JSON.stringify(criteria)}`,
    );
    ...
    const auditLogs = items.map((doc) =>
      this.auditLogMongoDBMapper.toAggregate(
        doc as unknown as AuditLogMongoDbDto,
      ),
    );

    return new PaginatedResult<AuditLogAggregate>(
      auditLogs,
      total,
      page,
      limit,
  ...
  async save(aggregate: AuditLogAggregate): Promise<AuditLogAggregate> {
    ...
    const collection = this.mongoService.getCollection(this.collectionName);
    const mongoData =
      this.auditLogMongoDBMapper.fromAggregateToMongoData(aggregate);

    await collection.replaceOne({ id: aggregate.id.value }, mongoData, {
      upsert: true,
    });
