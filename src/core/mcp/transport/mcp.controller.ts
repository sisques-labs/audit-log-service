 ...
 *
 * Exposed at `POST /mcp`. A new MCP server + transport is created per request
 * (no session state). The audit service is unauthenticated, so the tool context
 * is empty — tools only expose read-only queries.
 */
@Controller('mcp')
